...
def write_statistics_to_json():
    ...
    with open('ping_statistics.json', 'w') as json_file:
...
def main():
    global IP, PORT, PING_COUNT, count, count_of_received, rtt_sum, rtt_min, rtt_max

    try:
        opts, args = getopt.getopt(sys.argv[1:], "h:c:o:p:", ["hostname=", "count=", "options=", "port="])
    ...
    if not IP or not PORT:
        print("Usage: udpping.py -h <hostname> -c <count> -p <port>")
        sys.exit(2)
